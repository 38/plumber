...
int verify_token()
{
	pss_lex_token_t token;
	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_IDENTIFIER, CLEANUP_NOP);
	...
	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_KEYWORD && token.value.k == PSS_LEX_KEYWORD_ECHO, CLEANUP_NOP);
	ASSERT(token.line == 6, CLEANUP_NOP);
	...
	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_KEYWORD && token.value.k == PSS_LEX_KEYWORD_VISUALIZE, CLEANUP_NOP);
	ASSERT(token.line == 6, CLEANUP_NOP);
	ASSERT(token.offset == 26, CLEANUP_NOP);
	ASSERT_STREQ(token.file, TESTDIR"/test_pss.in", CLEANUP_NOP);

	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_KEYWORD && token.value.k == PSS_LEX_KEYWORD_START, CLEANUP_NOP);
	ASSERT(token.line == 6, CLEANUP_NOP);
	ASSERT(token.offset == 36, CLEANUP_NOP);
	ASSERT_STREQ(token.file, TESTDIR"/test_pss.in", CLEANUP_NOP);

	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_KEYWORD && token.value.k == PSS_LEX_KEYWORD_INCLUDE, CLEANUP_NOP);
	ASSERT(token.line == 7, CLEANUP_NOP);
	ASSERT(token.offset == 0, CLEANUP_NOP);
	ASSERT_STREQ(token.file, TESTDIR"/test_pss.in", CLEANUP_NOP);

	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_STRING, CLEANUP_NOP);
	...
	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_INTEGER && token.value.i == 223, CLEANUP_NOP);
	ASSERT(token.line == 8, CLEANUP_NOP);
	ASSERT(token.offset == 16, CLEANUP_NOP);
	ASSERT_STREQ(token.file, TESTDIR"/test_pss.in", CLEANUP_NOP);

	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_INTEGER && token.value.i == 0xff, CLEANUP_NOP);
	ASSERT(token.line == 9, CLEANUP_NOP);
	ASSERT(token.offset == 0, CLEANUP_NOP);
	ASSERT_STREQ(token.file, TESTDIR"/test_pss.in", CLEANUP_NOP);

	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_LBRACE, CLEANUP_NOP);
	ASSERT(token.line == 10, CLEANUP_NOP);
	ASSERT(token.offset == 0, CLEANUP_NOP);
	ASSERT_STREQ(token.file, TESTDIR"/test_pss.in", CLEANUP_NOP);

	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_RBRACE, CLEANUP_NOP);
	ASSERT(token.line == 10, CLEANUP_NOP);
	...
	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_LPARENTHESIS, CLEANUP_NOP);
	ASSERT(token.line == 12, CLEANUP_NOP);
	ASSERT(token.offset == 0, CLEANUP_NOP);
	ASSERT_STREQ(token.file, TESTDIR"/test_pss.in", CLEANUP_NOP);

	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_RPARENTHESIS, CLEANUP_NOP);
	ASSERT(token.line == 13, CLEANUP_NOP);
	ASSERT(token.offset == 0, CLEANUP_NOP);
	ASSERT_STREQ(token.file, TESTDIR"/test_pss.in", CLEANUP_NOP);

	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_GRAPHVIZ_PROP, CLEANUP_NOP);
	...
	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_LT, CLEANUP_NOP);
	ASSERT(token.line == 18, CLEANUP_NOP);
	ASSERT(token.offset == 0, CLEANUP_NOP);
	ASSERT_STREQ(token.file, TESTDIR"/test_pss.in", CLEANUP_NOP);

	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_GT, CLEANUP_NOP);
	ASSERT(token.line == 19, CLEANUP_NOP);
	ASSERT(token.offset == 0, CLEANUP_NOP);
	ASSERT_STREQ(token.file, TESTDIR"/test_pss.in", CLEANUP_NOP);

	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_SEMICOLON, CLEANUP_NOP);
	ASSERT(token.line == 20, CLEANUP_NOP);
	ASSERT(token.offset == 0, CLEANUP_NOP);
	ASSERT_STREQ(token.file, TESTDIR"/test_pss.in", CLEANUP_NOP);

	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_DOT, CLEANUP_NOP);
	ASSERT(token.line == 21, CLEANUP_NOP);
	ASSERT(token.offset == 0, CLEANUP_NOP);
	ASSERT_STREQ(token.file, TESTDIR"/test_pss.in", CLEANUP_NOP);

	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_COLON_EQUAL, CLEANUP_NOP);
	ASSERT(token.line == 22, CLEANUP_NOP);
	ASSERT(token.offset == 0, CLEANUP_NOP);
	ASSERT_STREQ(token.file, TESTDIR"/test_pss.in", CLEANUP_NOP);

	ASSERT_OK(pss_lex_next_token(lexer, &token), CLEANUP_NOP);
	ASSERT(token.type == PSS_LEX_TOKEN_ARROW, CLEANUP_NOP);
...
int teardown()
{
	ASSERT_OK(pss_lex_free(lexer), CLEANUP_NOP);
